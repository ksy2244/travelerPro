...
public class ReservationServlet extends TravelServlet {
	...
	protected void roomList(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (keyword.length() == 0) {
				list = dao.listRoom(offset, size);
			} else {
				list = dao.listRoom(offset, size, condition, keyword);
			}
			...
			// 페이징 처리
			String listUrl = cp + "/reservation/roomList.do";
			String articleUrl = cp + "/reservation/roomList.do?page=" + current_page;
			if (query.length() != 0) {
				listUrl += "?" + query;
				articleUrl += "&" + query;
			}
	...
	protected void roomInfo(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 게시물 가져오기
			ReservationDTO dto = dao.readRoom(companyNum);
