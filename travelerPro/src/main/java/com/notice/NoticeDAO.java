...
public class NoticeDAO {
	...
	public List<NoticeDTO> listNotice(int offset, int size) {
		...
		try {
			sql = "SELECT noticeNum, subject, reg_date, saveFilename " + " FROM notice " + " ORDER BY noticeNum DESC "
	...
	public NoticeDTO readNotice(long noticeNum) {
		...
		try {
			sql = "SELECT noticeNum, subject, content, TO_CHAR(reg_date, 'YYYY-MM-DD') reg_date, "
					+ " saveFilename, originalFilename, fileSize " + " FROM notice " + " WHERE noticeNum = ? ";
	...
	public void updateNotice(NoticeDTO dto) throws SQLException {
		...
		try {
			sql = "UPDATE notice SET subject=?, content=?, " + " saveFilename=?, originalFilename=?, fileSize=? "
					+ " WHERE noticeNum = ? ";
