...
public class QnaDAO {
	...
	public List<QnaVO> listQna(String userId, int offset, int size) {
		...
		try {
			sql = " SELECT questionNum, subject, "
					+ " TO_CHAR(reg_date, 'YYYY-MM-DD') reg_date ,categoryNum, userId, answer " + " FROM memberQ "
					+ " WHERE userId = ? " + " ORDER BY questionNum DESC " + " OFFSET ? ROWS FETCH FIRST ? ROWS ONLY ";
	...
	public List<QnaVO> listCategory() {
		...
		try {
			sql = " SELECT categoryNum, categoryName " + " FROM qnacategory ";
	...
	public QnaVO readQna(long questionNum) {
		...
		try {
			sql = " SELECT questionNum, subject, content, reg_date, q.userId, categoryNum " + " FROM memberQ q "
					+ " JOIN member m ON q.userId = m.userId " + " WHERE questionNum = ? ";
