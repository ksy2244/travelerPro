...
public class CeoMemberDAO {
	...
	public void insertMember(CeoMemberDTO dto) throws SQLException {
		// 사업자회원 등록
	...
	public void updateMember(CeoMemberDTO dto) throws SQLException {
		// 가입 후 회원 수정
		PreparedStatement pstmt = null;
		String sql;
		...
		try {
			sql = "UPDATE member SET userName=?, userPwd=?, nickname=?, "
					+ " modify_date=SYSDATE, birth=TO_DATE(?,'YYYYMMDD'), " + " email=?, tel=? WHERE userId=?";
			pstmt = conn.prepareStatement(sql);
