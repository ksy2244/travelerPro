...
public class RoomDAO {
	...
	public void updateroom(RoomDTO dto) throws SQLException {
		...
		try {
			...
			if (dto.getImageFiles() != null) {
				sql = "INSERT INTO roomFile(fileNum, imageFilename,roomNum) " + "VALUES(roomFile_seq.NEXTVAL, ?, ?)";
				pstmt = conn.prepareStatement(sql);
				for (int i = 0; i < dto.getImageFiles().length; i++) {
					pstmt.setString(1, dto.getImageFiles()[i]);
	...
	public List<RoomDTO> listroom(int offset, int size, long companyNum) {// int offset, int size
		...
		try {
			...
			sql = "SELECT roomNum, roomName, roomInfo, price, disCountRate, headCount, companyNum " + " FROM room "
					+ " WHERE companyNum=? " + " ORDER BY roomNum DESC " + " OFFSET ? ROWS FETCH FIRST ? ROWS ONLY ";
	...
	public RoomDTO readRoom(int roomNum) {
		...
		String sql;
		try {
			sql = "SELECT roomNum, roomName, roomInfo, price, discountRate, headCount " + " FROM room "
					+ " WHERE roomNum = ? ";
	...
	public List<companyDTO> selectCeo(long companyNum) {
		...
		String sql;
		try {
			sql = " SELECT companyNum, companyName " + " FROM company " + " WHERE  companyNum=? ";
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, companyNum);
